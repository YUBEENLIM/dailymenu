...
프로젝트는 백엔드(`src/`)와 프론트엔드(`flutter_app/`)를 **별도 브랜치**로 관리한다.

| 구분 | 브랜치 | 경로 |
|---|---|---|
| 백엔드 | `feature/backend` | `src/`, `build.gradle`, `application*.yml` |
| 프론트엔드 | `feature/frontend` | `flutter_app/` |
| 공통 (docs 등) | `main` 직접 | `docs/`, `CLAUDE.md`, `README.md` |

**규칙**

- `git add` 시 프론트/백 파일을 섞어서 staging하지 않는다
- 하나의 커밋에 `src/`와 `flutter_app/` 변경이 동시에 포함되면 안 된다
- 공통 파일(`docs/`, `CLAUDE.md`, `README.md`)은 `main`에 직접 커밋/push한다
- 공통 파일 push 후 feature 브랜치에서 `git rebase origin/main`으로 반영한다
- push 전에 현재 브랜치가 맞는지 확인한다